...
def get_rand(x, low, high) {
  ...
  x = ((rand % 16384) / 16384.) * (high - low) + low;
}
// --------------------------conv begins--------------------------------------------
...
vector[4] global_variance; //  = (0.1, 0.1, 0.2, 0.2);
...
// except h2 = PriorBox(feature_width, feature_height, mbox_priorbox, 4332, min_size, max_size, global_aspect_ratio, 5);
...
// except h3 = PriorBox(feature_width, feature_height, mbox_priorbox, 6732, min_size, max_size, global_aspect_ratio, 5);
...
// except h4 = PriorBox(feature_width, feature_height, mbox_priorbox, 7332, min_size, max_size, global_aspect_ratio, 5);
...
// except h5 = PriorBox(feature_width, feature_height, mbox_priorbox, 7548, min_size, max_size, global_aspect_ratio, 5);
...
// except h6 = PriorBox(feature_width, feature_height, mbox_priorbox, 7664, min_size, max_size, global_aspect_ratio, 5);
...
int total_num_det = 0;
vector[21][100] indices; // max valid scores of one class is 100
vector[21] num_dets; // how many valid scores of every class
...
for (i = 0; i < 21; ++i) {
  print "score index pairs ", i, " loops for ", num_dets[i];
...
vector[100][7] detection_out; //  [image_id, label, confidence, xmin, ymin, xmax, ymax]
